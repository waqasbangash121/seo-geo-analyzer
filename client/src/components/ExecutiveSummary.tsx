...
import { CrawlerRecommendations } from "@/components/CrawlerRecommendations";
...
export function ExecutiveSummary({ report }: { report: AuditReport }) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="flex flex-col items-center justify-center text-center lg:border-r border-border lg:pr-8">
            ...
            <p className="text-xs text-muted-foreground mt-3 flex items-center gap-1">
              <TrendingUp className="w-3.5 h-3.5" />~{potential} pts to reach an "A"
            </p>
          </div>
              ...
              {breakdown.map((b) => (
                <div key={b.label}>
                  ...
                  <div className="h-2 w-full rounded-full bg-muted overflow-hidden">
                    <div
                      className={`h-full rounded-full ${barColor(b.value)}`}
                      style={{ width: `${b.value}%` }}
                    />
                  </div>
                </div>
      ...
      <CrawlerRecommendations report={report.crawlerAudit} />
    </div>
  );
}
